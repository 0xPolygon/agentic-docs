...
Make sure you have followed the [steps in the quickstart](quickstart#set-up-environment-variables).
...
Follow the [start developing instructions](quickstart#start-developing) to spin up a local chain.
...
Internally, the [`dev`](commands#dev) command runs the `scripts/deploy_localhost` script that deploys all contracts in the correct sequence. 
...
Check and set the configs with [`dev`](commands#dev) command params `--ethernal-login-email`, `--ethernal-login-password` and `--ethernal-workspace`, which override the preset environment variables.

### Run the local block explorer

To use it, run the [`dev`](commands#dev) command with `-e`.

Access the chain explorer at the [Ethernal](https://app.tryethernal.com/) URL.

## Deploying to production

The [`deploy`](commands#deploy) command automates deploying to any EVM compatible chain. It runs the provided `scripts/deploy_prod` script to deploy all contracts in the correct sequence. When working on your own smart contracts, make sure to update this script.
