---
title: "Exit Roots"
description: "An exit tree is a binary, append-only, sparse Merkle tree (SMT) with a maximum depth of 32, where leaf nodes store bridging data. The Merkle root of an exit tre"
---
...
Data in a leaf contains a Keccak256 hash of the metadata (ABI encoded metadata if any) and the following parameters (matched by publicly available transaction data as seen in the [bridge L1 to L2](../../unified-LxLy/bridging#l1-to-l2) documentation):
...
<Callout type="info" title="Leaf parameters">
...
The system uses a set of [exit tree roots](exit-roots) to manage system state. Leaves of the trees point to transaction data such as detailed above.
...
<Callout type="tip" title="L1 or L2 update">
