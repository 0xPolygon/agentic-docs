---
title: "Processing L2 Blocks"
description: "In this document we discuss the differences between the Dragonfruit upgrade, which comes with the executor fork-ID 5, and the associated with fork-ID 6. The key"
---

In this document we discuss the differences between the Dragonfruit upgrade, which comes with the executor fork-ID 5, and the [Etrog upgrade](../protocol/etrog-upgrade) associated with fork-ID 6.
...
- As part of the block execution, zkEVM-specific parameters like each transaction's [$\texttt{effectivePercentage}$](../../architecture/effective-gas/tx-prioritization-egp) is included.
...
Each executed transaction includes a byte parameter called [$\texttt{effectivePercentage}$](../../architecture/effective-gas/tx-prioritization-egp), which is specific for Polygon zkEVM. This parameter is also stored in the $\mathtt{BlockInfoTree}$.
...
The $\mathtt{BlockInfoTree}$, being unique to each block, operates as a `Read`-`Write` sparse Merkle tree and functions similar to the [`L2StateTree`](../../architecture/proving-system/l2-state-tree) key-value structure.
