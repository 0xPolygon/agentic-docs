---
title: "Memory Sm"
description: "The Memory State Machine is a secondary state machine, therefore it has the executor part (the Memory SM executor) and an internal Memory PIL program which is a"
---
...
More cases are not possible because for an address we always start with a _write_ operation which limits the behavior of these constraints to these cases.
