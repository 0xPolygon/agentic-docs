...
description: "Although CIRCOM is mainly used to convert a STARK proof into its respective Arithmetic circuit, it is implemented in varied ways within the zkProver, specifical"
---
...
!!!info
    This document gives further elaboration on the [Proving architecture](proving-architecture) by focusing on the CIRCOM Arithmetic circuits, while highlighting CIRCOM's pivotal role in the STARK Recursion process.
...
As outlined in the [Proving architecture](proving-architecture) subsection, the recursive aggregation of many STARK proofs (aimed at reducing the size of the validity proof) is accomplished in five stages;
