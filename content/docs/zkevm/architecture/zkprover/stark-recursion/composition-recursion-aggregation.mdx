---
title: "Composition Recursion Aggregation"
description: "The STARK Recursion, as implemented in the zkEVM, is a specific method for combining multiple STARK proofs into a single proof. It is a process that involves; c"
---
...
As described in the PIL-STARK section of [this document](proving-tools), the basic verification of a STARK proof $\pi_`{STARK}`$ is performed by a Verifier entity which takes as inputs; the Proof, the Publics, and some other Verifier parameters.
...
CIRCOM is used as an intermediate representation language for the description of circuits. More details on how the CIRCOM is utilized is discussed in the [_Setup S2C_](recursion-sub-process) section.
...
See the [_Setup C2S_](recursion-sub-process) section for more details.
