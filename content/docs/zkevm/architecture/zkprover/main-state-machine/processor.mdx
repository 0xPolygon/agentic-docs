---
title: As a processor
description: As a microprocessor-type state machine, the main SM is a typical
  example of a _Generic state machine_. It is a state machine that behaves like
  an Algebraic proc
---
...
1. [Storage SM](../storage-state-machine/index) which handles all storage-related computations (e.g., Create, Read, Update and Delete).

2. [Arithmetic SM](../arithmetic-sm) which carries out elliptic curve arithmetic operations related to ECDSA.

3. [Binary SM](../binary-sm) which is responsible for performing all binary operations as the Executor requires.

4. [Memory SM](../memory-sm) which in the zkEVM plays the same role the EVM Memory plays in Ethereum.

5. [Keccak SM](../hashing-state-machines/keccakf-sm) which is a binary circuit that computes hash values of strings as instructed by the main SM. And, it is implemented within a special framework, detailed [here](../hashing-state-machines/keccak-framework).

6. [Poseidon SM](../hashing-state-machines/poseidon-sm) which specialises with computing hash values required in building Sparse Merkle trees as per the main SM instructions.

There are other *auxiliary* state machines used in the zkProver; the [Padding-KK](../hashing-state-machines/paddingkk-sm), the [Padding-KK-Bit](../hashing-state-machines/paddingkk-bit-sm), the Padding-PG SM, the [Memory Align SM](../mem-align-sm), the [Bits2Field SM](../hashing-state-machines/bits2field-sm) and the ROM SM ([sm_rom.js](https://github.com/0xPolygonHermez/zkevm-proverjs/blob/main/src/sm/sm_rom.js)).
...
The details on what these operations are, have been documented [here](../binary-sm).
