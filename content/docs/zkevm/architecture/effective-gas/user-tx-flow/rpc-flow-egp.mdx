---
title: RPC transaction flow
description: "The RPC flow phase of transactions consists of two stages: - The
  gas price suggestion. - Pre-execution of transactions. This flow ends with
  transactions being s"
---
...
4. Since there's no out of counters (OOC) error, the next step is to compute the $\texttt{BreakEvenGasPriceRPC}$.
    ...
    (Details on how this calculation are covered later in the [Implementing EGP strategy section](../implement-egp-strat).)
