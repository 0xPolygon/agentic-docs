---
title: "Plookup"
description: "The main state machine executor sends various instructions to the secondary state machines within the zkProver. Although secondary state machines specialize in "
---
...
Unless otherwise stated, all sorted multisets are henceforth sorted by $\mathbb{N}$, where $\mathbb{N}$ is the set of natural numbers $\{1, 2, 3, \dots \}$.
...
A test can therefore be coined using a _grand product argument_ akin to the one used in PLONK’s permutation argument [[GWC19]](https://eprint.iacr.org/2019/953).
...
In the generic state machine, recall [polynomials that define instructions](exec-trace-correct#polynomial-identities); $\texttt{CONST}$, $\texttt{inA}$, $\texttt{inB}$, $\texttt{inFREE}$, $\texttt{setA}$, $\texttt{setB}$, $\texttt{JMP}$, $\texttt{JMPZ}$, $\texttt{offset}$ and $\texttt{line}$.
