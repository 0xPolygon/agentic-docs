...
<Callout type="info" title="Active validator limit">
...
<Callout type="idea" title="Stay in the know">
...
<Callout type="info" title="PIP4 raised the minimum staking amount">
...
* Sync changes to Polygon staking contracts on Ethereum:
    ...
    * In this case, there is an `ack/no-ack` process that is followed to ensure that the next checkpoint contains a snapshot of the previous Bor blocks as well. For example, if checkpoint 1 is for Bor blocks 1-256, and it failed for some reason, the next checkpoint 2 will be for Bor blocks 1-512. See also [Heimdall v2 architecture: Checkpoint](../architecture/heimdall_v2/checkpoints).
* State sync from the Ethereum mainnet to Bor:
    ...
    * A similar mechanism is in place for sending state from Polygon to Ethereum. See also [State Sync Mechanism](../how-to/bridging/l1-l2-communication/state-transfer).
...
* [Start and run the nodes with Ansible](../how-to/validator/validator-ansible).
* [Start and run the nodes with binaries](../how-to/validator/validator-binaries).
* [Stake as a validator](../how-to/operate-validator-node/next-steps#stake-tokens).

### Maintain your validator nodes

* [Change the signer address](../how-to/operate-validator-node/change-signer-address).
* [Change the commission](../how-to/operate-validator-node/next-steps#changing-your-commission-rate).
