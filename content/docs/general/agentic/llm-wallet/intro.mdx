...
> **⚠️ Development Notice**: This package is still in development. Please prefer
> to use on testnet (polygon-amoy) or if using on mainnet, use only small
> amounts. Do not use in production environments.
...
- **`x402_check_payment`** - Pre-check if wallet can afford payment (before
  making it)
...
- **`seller_decode_payment`** - Decode payment header without calling
  facilitator
- **`seller_create_requirements`** - Generate payment requirements for protected
  resource
